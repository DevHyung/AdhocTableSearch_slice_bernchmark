...
class VerticalAttentionTableBertInputFormatter(VanillaTableBertInputFormatter):
    ...
    def get_input(self, context: List[str], table: Table):
        ...
        t_data = table[1].data
        t_header = table[1].header
        table_data = self.get_row_highest_ngram(context, t_data, n=2, k=self.config.sample_row_num)

        for row_data in table_data:
            if isinstance(row_data, dict):
                row_data = [row_data[col.name] for col in t_header]
            row_instance = self.get_row_input(context, t_header, row_data)
