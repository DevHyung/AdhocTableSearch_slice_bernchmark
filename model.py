...
class QueryTableMatcher(pl.LightningModule):
    ...
    def forward(self, query, columns, captions):
        ...
        #query = self.norm(self.Tmodel.bert(**query)[0]) # B x L x d
        ...
        for q, column, caption in zip(query, columns, captions):
            ...
            concat_encoding = self.norm(context_encoding[:, 0, :] + torch.mean(column_encoding, dim=1)) # Table representation # x768
            
            # calculate dot_product
            #score = self.norm(q) * self.norm(concat_encoding) # subN x 768

            # Multi-instance
            H = q * concat_encoding  # subN x 768
            
            # embedding max
            score, _ = torch.max(H, 0)   # T(# of table) x 768
            score_reps.append(score.mean(0))
        ...
        return self.linear(torch.stack(reps)), torch.stack(score_reps)  # B x 1

    def training_step(self, batch, batch_idx):
        query, columns, captions, rel = batch
        outputs, dot_scores = self(query, columns, captions)
        ...
        bce_loss = F.binary_cross_entropy_with_logits(outputs, rel.unsqueeze(1))
        mse_loss = F.mse_loss(dot_scores, rel)
